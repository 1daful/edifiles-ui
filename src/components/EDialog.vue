<template>
  ...
  export default {
    ...
    methods:
    {
      handlePan (details: { direction: string; evt: { preventDefault: () => void; stopPropagation: () => void }; isFirst: any; offset: { y: number }; isFinal: any }) {
        ...
        if (details.isFinal) {
          ...
          setTimeout((_: any) => this.$emit('input', false), 100)
          // reset modal position after transition
          setTimeout((_: any) => this.translateY = 0, 300)
        }
      },
    }
  }
